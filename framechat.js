...
<div class="age-overlay" id="ageOverlay">
  <div class="age-popup">
    ...
    <p class="age-footer">Para mais informações, consulte nossa <a href="https://frameag.com/privacy">Política de Privacidade</a> e <a href="https://frameag.com/termos">os Termos da Frame</a>.</p>
  </div>
</div>

<div class="media-confirm-overlay" id="mediaConfirmOverlay">
  <div class="media-confirm-popup" id="mediaConfirmPopup">
    <h2 id="mediaConfirmTitle"></h2>
    <img id="mediaConfirmPreview" class="media-confirm-preview" src="">
...
function sendMessage() {
  if (messageInput.value.trim()) {
    ...
    sentMessage.innerHTML = `${messageInput.value}<div class="message-time">agora mesmo</div>`;
    ...
    setTimeout(() => {
      messages.removeChild(digitando);
...
function showMediaConfirm(file) {
  ...
  reader.onload = (e) => {
    mediaConfirmTitle.textContent = `${window.currentProfile.name} Receberá sua mídia!`;
...
function closeMediaConfirm() {
  ...
  setTimeout(() => mediaConfirmOverlay.style.display = 'none', 600);
}
...
function handleMediaUpload(icon) {
  const input = document.createElement('input');
  input.type = 'file';
  input.accept = icon === 'gif' ? 'image/gif' : 'image/*';
...
ageAgreeBtn.addEventListener('click', () => {
  ...
  setTimeout(() => ageOverlay.style.display = 'none', 500);
});
