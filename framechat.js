...
// ===== Injetar CSS (estilo minimalista Apple-like com tons terrosos, animações leves, melhorias em responsividade) =====
...
@media (max-width: 768px) {
  ...
  .logo-img { width: 80px; }
  .conv-item { padding: 10px 14px; flex-wrap: wrap; }
  .conv-snippet { max-width: 150px; }
  .messages { padding: 14px; }
  .chat-input { padding: 10px 14px; flex-wrap: wrap; gap: 8px; }
  .input-icons { gap: 8px; }
  #send-btn { width: 100%; margin-left: 0; margin-top: 8px; }
  .popup { padding: 20px; }
...
@media (min-width: 769px) {
  #conversations, #chat-view { padding-left: 20%; padding-right: 20%; }
}
`;
...
// ===== Injetar HTML (estrutura minimalista para FrameChat, sem botão de fechar no popup) =====
const appHTML = `
...
// Dados dos perfis (5 iniciais gratuitos, apenas mensagens da criadora, melhoradas)
const profiles = [
  {
    name: 'Duda Brunet',
...
// Perfis blurred (mais itens, com overlay atraente como no print)
const blurredProfiles = [
  { name: 'Karol Rosalin', photo: 'https://example.com/karol.jpg', snippet: 'Vídeo inédito! Gozando gostoso...', time: 'há 1 dia' },
  { name: 'Reed', photo: 'https://example.com/reed.jpg', snippet: 'Estou em live agora...', time: 'há 3 dias' },
  { name: 'Gigenesi Nifree', photo: 'https://example.com/gigenesi.jpg', snippet: 'Novo post exclusivo...', time: 'há 2 dias' },
  { name: 'Brendatrindade', photo: 'https://example.com/brenda.jpg', snippet: 'Conteúdo 100% exclusivo...', time: 'há 4 dias' }
];
...
blurredProfiles.forEach((profile, index) => {
  const li = document.createElement('li');
  ...
  li.innerHTML = `
    ...
    <div class="blur-overlay">
      ...
      <div class="blur-text">Para ter acesso aos chats completos, torne-se Premium</div>
    </div>
  `;
  if (index === 0) {
    li.querySelector('.blur-overlay').addEventListener('click', showPremiumPopup);
  }
  convList.appendChild(li);
});
